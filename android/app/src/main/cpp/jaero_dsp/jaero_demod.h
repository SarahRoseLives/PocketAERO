...
/* Low-level decoded-frame callback (called after Viterbi + descramble).
 * data/len is the raw infofield bytes — useful for C-channel sub-band parsing. */
typedef void (*aerol_decoded_cb)(const uint8_t *data, int len, void *user);

/* AMBE voice frame callback — 12 bytes per frame, 25 frames per C-channel block */
typedef void (*aerol_voice_cb)(const uint8_t *ambe_frame, int frame_bytes, void *user);
...
void jaero_pmsk_set_decoded_callback(jaero_pmsk_demod_t *d, aerol_decoded_cb cb, void *user);
...
void jaero_oqpsk_cont_set_decoded_callback(jaero_oqpsk_cont_demod_t *d, aerol_decoded_cb cb, void *user);
void jaero_oqpsk_cont_set_voice_callback(jaero_oqpsk_cont_demod_t *d, aerol_voice_cb cb, void *user);
...
/* HilbertUSB IQ→audio converter — run at full RTL rate before decimation.
 * State persists across calls.  Produces int16 audio at the input sample rate. */
