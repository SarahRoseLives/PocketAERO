        ...
        else
        {
           ...
           // frame should be complete
            ...
            if(cntr==AERO_SPEC_NumberOfBits-1)
            {
                ...
                for(int y=0; y<24;y++)
                {
                    ...
                    if((int)infofield.size()==12)
                    {
                        ...
                        if(crcok)
                        {
                            ...
                            /* log C-channel sub-band unit */
                            {
                                static int ccnt = 0;
                                if (++ccnt <= 5 || ccnt % 100 == 0)
                                    __android_log_print(ANDROID_LOG_INFO, "RFStudio_SDR",
                                        "CSUB: crc_ok msg=%02X len=%d",
                                        (int)infofield.size() > 0 ? (unsigned)infofield[0] : 0,
                                        (int)infofield.size());
                            }
                        }
                ...
                /* --- Voice data extraction (port from JAERO Lines 2456-2491) ---
                 * 25 AMBE frames á 12 bytes (96 bits) follow the sub-band units.
                 * Frames are packed from raw deconvol bits, skipping 13 bits
                 * between each 96-bit block (FEC/CRC). */
                if (voice_cb) {
                    std::vector<uint8_t> vdata;
                    vdata.reserve(300);   /* 25 frames × 12 bytes */
                    uint8_t vch = 0;
                    int vbits = 0, vcharptr = 0;
                    for (int h = 1; h < 2714; h++) {
                        ...
                        if (vbits == 96) {
                            vbits = 0;
                            h += 13; /* skip 13 FEC bits between frames */
                        }
                    }

                    /* Send 25 individual 12-byte AMBE frames to callback */
                    for (int i = 0; i < 25 && (i + 1) * 12 <= (int)vdata.size(); i++) {
                        voice_cb(vdata.data() + i * 12, 12, voice_user);
                    }
                }
