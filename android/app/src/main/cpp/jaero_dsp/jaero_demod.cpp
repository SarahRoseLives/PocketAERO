                                       ...
                                       jaero_soft_bits_cb cb, void *user)
{
    ...
    s.Fs = sample_rate;
    s.fb = symbol_rate;
    /* Match OQPSK's 8 kHz center frequency.  NCO shifts signal → 1800 Hz,
     * Hilbert USB shifts by 8000 Hz → 1800-8000=-6200 (wraps in audio).
     * Wide AFC + lockingbw catches the actual signal band. */
    s.freq_center = 8000.0;
...
static void pmsk_decoded_adapter(const uint8_t *data, int len, void *ctx)
{
    jaero_pmsk_demod_t *d = (jaero_pmsk_demod_t *)ctx;
    if (d->decoded_cb)
        d->decoded_cb(data, len, d->decoded_user);
}

void jaero_pmsk_set_decoded_callback(jaero_pmsk_demod_t *d,
                                      aerol_decoded_cb cb, void *user)
{
    if (!d) return;
    d->decoded_cb = cb;
    d->decoded_user = user;
...
static void oqpsk_cont_voice_adapter(const uint8_t *ambe_frame, int frame_bytes, void *ctx);
                                                    ...
                                                    jaero_soft_bits_cb cb, void *user)
{
    ...
    /* Wire decoded-frame and voice callbacks to AeroL */
    d->aerol->setDecodedCallback(oqpsk_cont_decoded_adapter, d);
    d->aerol->setVoiceFrameCallback(oqpsk_cont_voice_adapter, d);
...
static void oqpsk_cont_voice_adapter(const uint8_t *ambe_frame, int frame_bytes, void *ctx)
{
    jaero_oqpsk_cont_demod_t *d = (jaero_oqpsk_cont_demod_t *)ctx;
    if (d->voice_cb)
        d->voice_cb(ambe_frame, frame_bytes, d->voice_user);
}
...
void jaero_oqpsk_cont_set_voice_callback(jaero_oqpsk_cont_demod_t *d,
                                           aerol_voice_cb cb, void *user)
{
    if (!d) return;
    d->voice_cb   = cb;
    d->voice_user = user;
    /* adapter is already wired in create() */
}
...
/* HilbertUSB IQ→audio converter (single static instance for the AERO feed path).
 * Runs at the full RTL-SDR IQ rate to match SDR++'s full-rate USB demodulation
 * before decimation.  State persists across calls for continuous phase. */
static HilbertUSB g_hilbert_usb;

void jaero_hilbert_usb_process(const double *iq_interleaved, int num_samples,
                                double Fs, double freq_center, double gain,
                                int16_t *out)
{
    g_hilbert_usb.process(iq_interleaved, num_samples, Fs, freq_center, gain, out);
}
